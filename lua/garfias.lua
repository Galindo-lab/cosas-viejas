...
function math.ratan(v) return math.atan(math.rad(v)) end
function math.rcos(v) return math.cos(math.rad(v)) end
function math.rsin(v) return math.sin(math.rad(v)) end
...
function plr_rct(amp,face)

   if amp < 0 then
      print("La amplitud no puede ser negativa")
   else
      sprintf("resultado: (%.4f) + (%.4f)i",
	      amp*math.rcos(face),
	      amp*math.rsin(face)
      )
   end
...
function rct_plr(rl,img,verbose)
   ...
   if verbose then
      sprintf("r: sqrt( %.4f ) θ: ( %.4f° + %.4f° )",r,i,qdr(rl,img))
   end
